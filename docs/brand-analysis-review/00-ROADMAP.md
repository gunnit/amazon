...
**2026-06-21 product decision (amended 2026-07-27):** Brand Analysis is independent. Its production path is Inthezon internal Amazon/SP-API data plus Market Research enrichment, with generic manual yearly export upload as fallback. Legacy `helium10*` mode names remain backward-compatible aliases for old rows and still route to manual upload — Helium 10 is **not** a Brand Analysis input. See [Brand Analysis — Independent Product Decision](../brand-analysis-independent.md).

**Amendment (2026-07-27, customer-confirmed):** the blanket "no Helium 10 integration" is narrowed to Brand Analysis inputs. A **complementary, fill-only Helium 10 integration in Market Research** is confirmed with the customer and in the tree — see **C-11** below.
...
| **C-11** | **Helium10 / H10 integration scope** | Older notes and compatibility types mention Helium10/H10. Gioia's sheet still carried "Dati H10 automaticamente caricati" as an open item. The 2026-06-21 ruling was a blanket "no H10 integration"; the customer has since asked for it back and it was confirmed to her by email (2026-07). | **Superseded 2026-07-27 — split by surface.** **(a) Brand Analysis inputs: still no H10.** Brand Analysis stays Inthezon-native; "automatic H10 import" remains closed by product scope; generic manual export upload is the only fallback and `helium10*` mode aliases keep routing to manual upload. **(b) Market Research: complementary fill-only H10 integration, shipped in tree.** Scope is deliberately narrow and matches what was promised to the customer: **non-owned (competitor) ASINs only**, and only for fields SP-API structurally cannot return for them — rating, review count, estimated monthly sales, estimated revenue, BSR, and price as a last-resort fallback. It is a **gap-filler, not a dependency and not a replacement**: Amazon values always win, only `None` fields are touched, every filled field is recorded in `helium10_fields` provenance because H10 values are modeled daily estimates rather than live data, and the whole thing is off unless a credential is configured. Quota is a shared 10k calls/month across the H10 account, so calls are capped per market search and cached 24h; owned ASINs are explicitly excluded (`_fetch_product_data(..., enrich_third_party=False)`). Code: `backend/app/core/helium10.py`, wired at `market_research_service._fetch_product_data` and `api/v1/market_research.py` market-search. **Caveat:** H10's MCP server is OAuth-2.1-only ("API Token is not supported yet"), so unattended backend use needs an admin-pasted access token until H10 ships token auth; the response schema is still unvalidated pending a live probe. Relative priority is **low** — the customer explicitly ranked H10 listing/keyword data well below her primary ask. |
